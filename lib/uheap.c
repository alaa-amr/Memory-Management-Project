...
void halfLast(){
	...
	int in=-1,a=0;
	for(int i=size_of_alloc-1;i>=0;i--){
		if(allocatd_frames[i].size != -1){
			in=i;
			lastadd=allocatd_frames[i].address;
			a =allocatd_frames[i].num;
			break;
		}
	}
	int new_size=((allocatd_frames[in].size)/2);
	free((void*)lastadd);
	new_size=ROUNDUP(new_size,PAGE_SIZE);
	int numOfBlocks = ROUNDUP(new_size,PAGE_SIZE) / PAGE_SIZE;
	for (int i = 0 ; i < numOfBlocks ; i++  )
			{

				user_heap_frames[a+i] = 1 ;

			}
			sys_allocateMem(lastadd,new_size);

			allocatd_frames[in].size=new_size;
			allocatd_frames[in].address=lastadd;

			allocatd_frames[size_of_alloc].num=a;

...
void placement(struct Env * curenv, uint32 fault_va){
  ...
  LIST_FOREACH(iterator, &(curenv->SecondList))
  {
    if(iterator->virtual_address == fault_va){
      ...
      pt_set_page_permissions(curenv, element->virtual_address, 0, PERM_PRESENT);
      LIST_INSERT_HEAD(&(curenv->ActiveList), iterator);
      //set its present bit by 1
      pt_set_page_permissions(curenv, iterator->virtual_address, PERM_PRESENT, 0);
      //pt_set_page_permissions(curenv, iterator->virtual_address, PERM_USER, 0);
      //pt_set_page_permissions(curenv, iterator->virtual_address, PERM_WRITEABLE, 0);
      return;
    }
  }
   ...
   map_frame(curenv->env_page_directory, ptr_frame_info, (void*)fault_va, PERM_PRESENT|PERM_WRITEABLE|PERM_USER);

    int read_ret = pf_read_env_page(curenv, (void*)fault_va);
    if (read_ret == E_PAGE_NOT_EXIST_IN_PF ){
      if(fault_va >= USTACKBOTTOM && fault_va <= USTACKTOP){
        int add_ret = pf_add_empty_env_page(curenv, fault_va, 1);
        if (add_ret == E_NO_PAGE_FILE_SPACE)
        panic("ERROR: No enough virtual space on the page file");
      }
      else
      {
        panic("ERROR: the page isn't found in the page file and isn't a stack page");
      }
    }
      if( LIST_SIZE(&(curenv->ActiveList))== curenv->ActiveListSize){
        struct WorkingSetElement* element = LIST_LAST(&(curenv->ActiveList));
        LIST_REMOVE(&(curenv->ActiveList), element);
        if(curenv->SecondListSize){
        LIST_INSERT_HEAD(&(curenv->SecondList), element);
        pt_set_page_permissions(curenv, element->virtual_address,0,PERM_PRESENT);
        }
        else{
          LIST_INSERT_HEAD(&(curenv->PageWorkingSetList), element);
          uint32 page_permissions = pt_get_page_permissions(curenv, element->virtual_address);
                  if(page_permissions & PERM_MODIFIED)
                  {
                    uint32* ptr_page_table;
                    struct Frame_Info *ptr_frame_info = get_frame_info(curenv->env_page_directory,
                    (void*)element->virtual_address, &ptr_page_table);
                    if(pf_update_env_page(curenv, (void*)element->virtual_address, ptr_frame_info)== E_PAGE_NOT_EXIST_IN_PF){
                      panic("ERROR: the page isn't found in the page file");
                    }
                  }
      ...
      struct WorkingSetElement* WS_element = curenv->PageWorkingSetList.lh_last;
...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
  ...
  //if there are elements in the pageworkingset
	fault_va = ROUNDDOWN(fault_va,PAGE_SIZE);
  int ws_curr_size = LIST_SIZE(&(curenv->PageWorkingSetList));
  if(ws_curr_size){
    placement(curenv, fault_va);
  }
  // if there aren't elements in the working set
  else{
    int found = 0;
    struct WorkingSetElement* iterator;
    LIST_FOREACH(iterator, &(curenv->SecondList)){
    ...
    if(!found){
      ...
      if(curenv->SecondListSize){
    	  ...
    	  LIST_INSERT_TAIL(&(curenv->PageWorkingSetList), victim);
        ...
        if(page_permissions & PERM_MODIFIED)
        {
          ...
          if(pf_update_env_page(curenv, (void*)victim->virtual_address, ptr_frame_info)== E_PAGE_NOT_EXIST_IN_PF){
            panic("ERROR: the page isn't found in the page file");
          }
        }
